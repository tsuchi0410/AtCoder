...
int main(){
    ...
    dp[0][2] = R[0];
    rep(i, N - 1){
        chmin(dp[i + 1][0], dp[i][0] + P[i + 1] - A);
        chmin(dp[i + 1][0], dp[i][1] + P[i + 1]);
        chmin(dp[i + 1][0], dp[i][2] + P[i + 1]);

        chmin(dp[i + 1][1], dp[i][0] + Q[i + 1]);
        chmin(dp[i + 1][1], dp[i][1] + Q[i + 1] - A);
        chmin(dp[i + 1][1], dp[i][2] + Q[i + 1]);

        chmin(dp[i + 1][2], dp[i][0] + R[i + 1]);
        chmin(dp[i + 1][2], dp[i][1] + R[i + 1]);
        chmin(dp[i + 1][2], dp[i][2] + R[i + 1] - A);        
    }
