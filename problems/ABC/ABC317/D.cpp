...
int main(){
    ...
    ll num = 100010;
    vvl dp(len(lst), vl(num, INF));
    rep(i, len(lst)){
        rep(j, num){
            if(j == lst[i][1]){
                chmin(dp[i][j], lst[i][0]);
            }
            if(i + 1 == len(lst)){
                continue;
            }
            if(dp[i][j] != INF){
                ...
                chmin(dp[i + 1][j + lst[i + 1][1]], dp[i][j] + lst[i + 1][0]);
            }
    ...
    ll ans = INF;
    rep(i, num){
        if(chokudai + i > aoki - i){
            chmin(ans, dp[len(lst) - 1][i]);
        }
    }
    print(ans);
}