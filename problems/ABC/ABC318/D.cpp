...
int main(){
    ...
    // dp[i] : 頂点がどこかしらに辺を張っているか (2 ^ N)
    vl dp(1 << N);

    // 遷移
    // 現状張っている辺は 1, 張っていない辺は 0 で表す
    // 例えば、 "0101" のとき、辺 0 と辺 2 が繋がっている
    // "1111" のときは、全てどこかしらの辺と繋がっている
    // 新しく辺を繋げるときは 2 つの頂点が必要なので、
    // "0101" -> "1111" のように遷移する
    rep(s, 1 << N){
        rep(i, N - 1){
            if(s >> i & 1 == 1){
            ...
            rep(j, i + 1, N){
                ...
                // cout << bitset<4>(t) << endl;

                chmax(dp[t], dp[s] + D[i][j - i - 1]);
            }
        }
    }
