...
ll u;
bool dfs(long long v, vector<vector<long long>> &G, vector<bool> &seen, vector<bool> &finished) {
    bool f = false;
    seen[v] = true;
    for(auto v2 : G[v]) {
        if(seen[v2] == true) {
            if(finished[v2] == false) {  // 行きがけ順のときに未探索な帰りがけの頂点を見つけた
                u = v2;
                return true;
            }
            continue;
        }
        f |= dfs(v2, G, seen, finished); 
    }
    // 帰りがけ
    finished[v] = true;
    return f;
...
int main(){
    ...
    rep(v, N){
        ...
        bool cycle = dfs(v, G, seen, finished);
        if(cycle == true){
            vl path;
            ll now = u;
            do{
