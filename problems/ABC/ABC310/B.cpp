...
// 部分集合判定 s1 <= s2
bool is_subset(const std::unordered_set<long long>& set1, const std::unordered_set<long long>& set2) {
...
int main(){
    ...
    rep(i, N){
        rep(j, N){
            ...
            if((P[i] > P[j]) and (is_subset(F[i], F[j]))){
                print("Yes");
                return 0;
            }
            if ((P[i] == P[j]) and (is_subset(F[i], F[j])) and (len(F[i]) < len(F[j]))){
                print("Yes");
                return 0;
            }
