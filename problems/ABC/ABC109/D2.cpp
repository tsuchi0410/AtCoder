...
#define RREP1(i, n) for (auto i = (n); (i) != (-1); --(i))
#define RREP2(i, l, r) for (auto i = (l); (i) != (r); --(i))
#define RREP3(i, l, r, s) for (auto i = (l); (i) > (ll)(r); (i) -= s)
...
int main(){
    ...
    rep(i, H){
        if(i % 2 == 0){
            rep(j, W){
                if(G[i][j] % 2 == 1 and rec == false){
                    rec = true;
                }elif(G[i][j] % 2 == 1 and rec == true){
                    l.pb({i + 1, j + 1});
                    ans.pb(l);
                    l.clear();
                    rec = false;
    ...
    rep(i, len(ans)){
        rep(j, len(ans[i]) - 1){
            print(ans[i][j][0], ans[i][j][1], ans[i][j + 1][0], ans[i][j + 1][1]);
        }
    }
}