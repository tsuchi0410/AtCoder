...
int main(){
    ...
    fore(i, v){
        ll la, lq;
        la = bisect_left(m['A'], i);
        lq = bisect_left(m['?'], i);
        mint a;
        a = mint(la) * mint(3).pow(lq) + mint(lq) * mint(3).pow(max(ll(0), lq - 1));
        ll rc, rq;
        rc = len(m['C']) - bisect_left(m['C'], i);
        rq = len(m['?']) - bisect_right(m['?'], i);
        mint c = mint(rc) * mint(3).pow(rq) + mint(rq) * mint(3).pow(max(ll(0), rq - 1));
        ans += a * c;
    }
    print(ans.val());
