...
template<class Monoid, class Action> struct LazySegmentTree {
  ...
  void init(const vector<Monoid> &v,
            ...
            const Monoid &identity_monoid,
            const Action &identity_action) {
    init((ll)v.size(), op, mapping, composition, identity_monoid, identity_action);
    build(v);
  }
  void build(const vector<Monoid> &v) {
    assert(N == (ll)v.size());
    for (ll i = 0; i < N; ++i) dat[i + offset] = v[i];
    for (ll k = offset - 1; k > 0; --k) pull_dat(k);
  }
  ll size() const {
    return N;
  }
  ...
  void apply_lazy(ll k, const Action &f) {
  ...
  void pull_dat_deep(ll k) {
    for (ll h = 1; h <= log; ++h) pull_dat(k >> h);
  }
  void push_lazy_deep(ll k) {
    for (ll h = log; h >= 1; --h) push_lazy(k >> h);
  }
  
  // setter and getter, update A[i], i is 0-indexed, O(log N)
  void set(ll i, const Monoid &v) {
    assert(0 <= i && i < N);
    ll k = i + offset;
    push_lazy_deep(k);
    dat[k] = v;
  ...
  Monoid operator [] (ll i) {
    return get(i);
  }
  
  // apply f for index i
  void apply(ll i, const Action &f) {
    assert(0 <= i && i < N);
    ll k = i + offset;
    push_lazy_deep(k);
    dat[k] = MAPPING(f, dat[k]);
  ...
  // apply f for llerval [l, r)
  void apply(ll l, ll r, const Action &f) {
    assert(0 <= l && l <= r && r <= N);
    if (l == r) return;
    l += offset, r += offset;
    for (ll h = log; h >= 1; --h) {
  ...
  // get prod of llerval [l, r)
  Monoid prod(ll l, ll r) {
    assert(0 <= l && l <= r && r <= N);
  ...
  ll max_right(const function<bool(Monoid)> f, ll l = 0) {
    if (l == N) return N;
    l += offset;
    push_lazy_deep(l);
  ...
  // f(IDENTITY) need to be True
  ll min_left(const function<bool(Monoid)> f, ll r = -1) {
    if (r == 0) return 0;
    if (r == -1) r = N;
    r += offset;
    push_lazy_deep(r - 1);
  ...
  friend ostream& operator << (ostream &s, LazySegmentTree seg) {
    for (ll i = 0; i < (ll)seg.size(); ++i) {
      s << seg[i];
      if (  i != (ll)seg.size() - 1) s << " ";
    }
    return s;
  }
...
struct Node {
  ll val;
  long long siz;
  Node(ll v = 0, long long s = 0) : val(v), siz(s) {}
};
...
int main(){
  ...
  rep(_, Q){
    ...
    if(t == 1){
      LL(x);
      seg.apply(0, N, Act(0, x));
    }else if(t == 2){
      LL(i, x);
      i--;
      seg.apply(i, i + 1, Act(1, x));
    }else{
      LL(i);
      i--;
      Node res = seg.prod(i, i + 1);
